...
try {
  ...
  const localLoginResponse = await fetch(`${baseUrl}/login`, { redirect: "manual" });
  assert.equal(localLoginResponse.status, 404, "local dashboard must not expose server-only login when auth is disabled");
