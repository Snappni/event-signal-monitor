...
class ResourceTests(unittest.TestCase):
    ...
    def test_supervisor_preserves_report_and_cleans_tree(self):
        for scenario in ['preflight', 'probe_error', 'mine', 'running', 'failed', 'success']:
            with self.subTest(scenario=scenario), tempfile.TemporaryDirectory() as tmp:
                ...
                child.poll.return_value = None if scenario=='running' else child.returncode
                ...
                with patch.dict(os.environ, {'FACTOR_RESEARCH_PROFILE':'server-low', 'FACTOR_RESEARCH_TASK_ID':'fixture'}), \
                        ...
                        patch.object(rb.os, 'nice', create=True), patch.object(rb.os, 'getppid', return_value=123), \
                ...
                self.assertEqual(json.loads(report.read_text()), {'new':True} if code==0 else {'old':True})
                self.assertFalse((root/'report-fixture.pending.json').exists())
                self.assertEqual(start.call_count, int(scenario in ['running','failed','success']))
